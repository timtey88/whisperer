...
export function useSingleInstance({ setFiles }: UseSingleInstanceProps) {
	...
	async function handleSingleInstance() {
		...
		await listen<string[]>('single-instance', async (event) => {
			...
			// whisperer://download/?url=google.com
			// already handled in deep links in macos
			if (action && action.startsWith('whisperer://download/?') && platform != 'macos') {
				const params = new URLSearchParams(action.replace('whisperer://download/?', ''))
				const url = params.get('url')
				if (url) {
					const downloadURL = url.replace('whisperer://download/?url=', '')
